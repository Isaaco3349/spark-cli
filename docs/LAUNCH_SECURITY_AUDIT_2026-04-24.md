...
- spark-telegram-bot is private by default; only `/start` and `/myid` are public onboarding commands unless `ALLOWED_TELEGRAM_IDS` or `TELEGRAM_PUBLIC_CHAT_ENABLED=1` is configured. Webhook secrets are length/charset validated and compared in constant time.
...
- P1 fixed: webhook secret comparison used direct string equality. It now validates the token and compares in constant time.
...
- P2 open: no automated tests yet. `npm test` is a no-op; this repo needs focused tests for access-gate behavior, webhook secret validation, and `/myid` onboarding.
- P2 open: webhook server binds to `0.0.0.0` by design for public ingress. This must remain behind Telegram secret-token validation and a tunnel/proxy policy.
...
- `TELEGRAM_WEBHOOK_SECRET` must be 16-256 chars and only `A-Z`, `a-z`, `0-9`, `_`, `-`.

Recommended next tests:

- Add unit tests around `ConversationMemory.isAllowed`.
- Add a webhook smoke test for invalid/missing/valid secret header.
- Start in smoke mode with a valid secret and verify `/healthz` plus unauthorized webhook POST.
